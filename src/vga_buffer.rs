...
// We need to use lazy static to make Writer static because Rust's const
// evaluator is not able to convert raw pointers to references at compile
// time. Instead the static will be initialized when it is first accessed
// at runtime.
lazy_static! {
...
impl Writer {
    ...
    /// Clear a row by overwriting it with blank bytes.
    ...
    /// Write a new line byte to the buffer. A new line is inserted by
    /// shifting all lines one line up and clearing the last row.
...
/// Print macro for printing text to the screen without a new line.
#[macro_export]
macro_rules! print {
    ($($arg:tt)*) => ($crate::vga_buffer::_print(format_args!($($arg)*)));
}

/// Println macro for printing text to the screen with a new line.
#[macro_export]
macro_rules! println {
    () => ($crate::print!("\n"));
    ($($arg:tt)*) => ($crate::print!("{}\n", format_args!($($arg)*)));
...
/// Print the formatted string to the VGA buffer using the static `WRITER` instance.
#[doc(hidden)]
